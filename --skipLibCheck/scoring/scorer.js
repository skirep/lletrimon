...
exports.extractSoundToken = extractSoundToken;
exports.calculateSimilarity = calculateSimilarity;
exports.calculateSyllableSimilarity = calculateSyllableSimilarity;
exports.classifyResult = classifyResult;
exports.detectErrors = detectErrors;
exports.calculateScore = calculateScore;
exports.calculateXpGained = calculateXpGained;
const SPOKEN_LETTER_MAP = {
    a: 'a',
    e: 'e',
    i: 'i',
    o: 'o',
...
const SPOKEN_BIGRAM_LETTER_MAP = {
    'i grega': 'y',
    'doble ve': 'w',
};
 ...
 *  - "a" -> "a"
 */
function extractSoundToken(recognized) {
    ...
    for (const word of words) {
        if (/^[a-z]$/.test(word))
            return word;
        if (SPOKEN_LETTER_MAP[word])
            return SPOKEN_LETTER_MAP[word];
    }
...
function calculateSimilarity(expected, recognized) {
...
function calculateSyllableSimilarity(expected, recognized) {
    ...
    for (const word of recognizedWords) {
        ...
        bestSimilarity = Math.max(bestSimilarity, calculateSimilarity(normalizedExpected, prefix), calculateSimilarity(normalizedExpected, suffix));
    }
    return bestSimilarity;
}
function classifyResult(similarity) {
    if (similarity >= 0.8)
        return 'correct';
    if (similarity >= 0.55)
        return 'almost';
...
function detectErrors(expected, recognized) {
    const errors = [];
    const a = normalize(expected);
    const b = normalize(recognized);
    if (!a || !b)
        return errors;
    // b/d confusion
    const swapBD = (s) => s.replace(/b/g, 'X').replace(/d/g, 'b').replace(/X/g, 'd');
    if (normalize(swapBD(a)) === b || normalize(swapBD(b)) === a) {
        errors.push('b_d_confusion');
    }
    // p/q confusion
    const swapPQ = (s) => s.replace(/p/g, 'X').replace(/q/g, 'p').replace(/X/g, 'q');
...
function calculateScore(correctItems, totalItems) {
    if (totalItems === 0)
        return 0;
    return Math.round((correctItems / totalItems) * 100);
}
function calculateXpGained(score, difficulty, timeMs) {
